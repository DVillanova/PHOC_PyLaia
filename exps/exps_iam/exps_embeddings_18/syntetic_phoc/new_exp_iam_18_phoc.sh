...
TextDir=${HomeDataDir}/18_PHOC_TEXT
LangDir=${HomeDataDir}/18_PHOC_TEXT
CharDir=${LangDir}/char
ModelDir=${HomeDataDir}/18_PHOC_model
TmpDir=${HomeDataDir}/18_PHOC_TMP
...
nice pylaia-htr-decode-ctc \
  --common.train_path ${ModelDir} \
  --common.model_filename model \
  --common.checkpoint ${ModelDir}/experiment/epoch=276-lowest_va_ecer.ckpt \
  --logging.level INFO \
  --logging.to_stderr_level INFO \
...
ie-eval all --label-dir ${TmpDir}/bio_labels/ --prediction-dir ${TmpDir}/decode/bio_labels/ > ${HomeDataDir}/results_18_phoc.txt

ie-eval nerval --label-dir ${TmpDir}/bio_labels/ \
--prediction-dir ${TmpDir}/decode/bio_labels/ \
>> ${HomeDataDir}/results_18_phoc.txt

cat $HomeDataDir/results_18_phoc.txt
...
python $ScriptsDir/compute-cer_wer.py $TmpDir/test_no_tags.txt $TmpDir/decode/wordtest_no_tags.txt >> $HomeDataDir/results_18_phoc.txt
cat $HomeDataDir/results_18_phoc.txt
...
#Evaluation
echo "\n\nWith 6-gram character LM" >> $HomeDataDir/results_18_phoc.txt
...
$TmpDir/decode/with_char_lm/wordtest_no_tags.txt >> $HomeDataDir/results_18_phoc.txt
cat $HomeDataDir/results_18_phoc.txt