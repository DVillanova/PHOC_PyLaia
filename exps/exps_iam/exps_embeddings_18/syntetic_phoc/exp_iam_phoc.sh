...
PartDir=${WorkDir}/iam/PARTITIONS/
HomeDataDir=/data2/dvillano/iam/
...
python $ScriptsDir/IAM_generate_tagged_line_transcription.py 3 \
       $HomeDataDir/ne_annotations/custom/iam_all_custom_6_all.txt $DataDir/ascii/lines.txt \
       $TextDir/index.words
...
#Generation of symb.txt file with symbols from CONLL and IAM
cp char.total.txt $HomeDataDir/../conll2003/synthetic_lines/all/char.iam.txt
cd $HomeDataDir/../conll2003/synthetic_lines/all/
for p in train valid iam; do cat char.${p}.txt | cut -f 2- -d " "  |
...
#Extract vocabulary and number of symbols in vocabulary from CONLL and IAM
for p in train valid iam; do cat char.${p}.txt | cut -f 2- -d " "  |
tr \  \\n; done | sort -u -V | awk 'BEGIN{   N=0;   printf("%-12s %d\n", "<eps>", N++);   printf("%-12s %d\n", "<ctc>", N++);  }NF==1{  printf("%-12s %d\n", $1, N++);}' >  ${CharDir}/symb.txt
...
python $ScriptsDir/IAM_generate_part_files.py $HomeDataDir/ne_annotations/custom/iam_train_custom_6_all.txt $TextDir/index.words $PartDir/train.lst
python $ScriptsDir/IAM_generate_part_files.py $HomeDataDir/ne_annotations/custom/iam_test_custom_6_all.txt $TextDir/index.words $PartDir/test.lst
python $ScriptsDir/IAM_generate_part_files.py $HomeDataDir/ne_annotations/custom/iam_valid_custom_6_all.txt $TextDir/index.words $PartDir/val.lst
...
#TODO: PROBAR A REDUCIR EL NUMERO DE UNIDADES 256 -> 128 
...
img_dirs="[/data/dvillano/conll2003/synthetic_lines/all/lines/]"
...
nice pylaia-htr-decode-ctc \
  --common.train_path ${ModelDir} \
  --common.model_filename model \
  --common.checkpoint ${ModelDir}/experiment/epoch=659-lowest_va_ecer.ckpt \
  --logging.level INFO \
  --logging.to_stderr_level INFO \
