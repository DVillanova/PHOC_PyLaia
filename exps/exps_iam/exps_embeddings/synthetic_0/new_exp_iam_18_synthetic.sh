...
nice pylaia-htr-decode-ctc \
  --common.train_path ${ModelDir} \
  --common.model_filename model \
  --common.checkpoint ${ModelDir}/experiment/epoch=304-lowest_va_ecer.ckpt \
  --logging.level INFO \
  --logging.to_stderr_level INFO \
...
ie-eval all --label-dir ${TmpDir}/bio_labels/ --prediction-dir ${TmpDir}/decode/bio_labels/ > ${HomeDataDir}/results_synthetic_18.txt

ie-eval nerval --label-dir ${TmpDir}/bio_labels/ \
--prediction-dir ${TmpDir}/decode/bio_labels/ \
>> ${HomeDataDir}/results_synthetic_18.txt

cat $HomeDataDir/results_synthetic_18.txt
...
python $ScriptsDir/compute-cer_wer.py $TmpDir/test_no_tags.txt $TmpDir/decode/wordtest_no_tags.txt >> $HomeDataDir/results_synthetic_18.txt
cat $HomeDataDir/results_synthetic_18.txt
...
#Evaluation
echo "\n\nWith 6-gram character LM" >> $HomeDataDir/results_synthetic_18.txt
...
$TmpDir/decode/with_char_lm/wordtest_no_tags.txt >> $HomeDataDir/results_synthetic_18.txt
cat $HomeDataDir/results_synthetic_18.txt