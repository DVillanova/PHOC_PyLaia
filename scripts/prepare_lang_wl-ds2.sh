...
[ $# -lt 4 ] && {
    echo "Usage: ${0##*/} <work-dir> <charMap-File> <blank-symb> <text> [<whiteSpace-symb>] [<Dummy-symb>]" >&2;
    exit 1;
}
...
#awk '{for(i=1;i<=NF;++i) print $i}' ${TEXT} | sort -u |
#awk -v cm="${CM_FILE}" 'BEGIN{while (getline < cm)  { L=length($1); p=substr($1,1,1); for (l=1;l<=L;l++) chrMap[substr($1,l,1)]=p }}{printf("%-20s 1.0  ", $0); for(i=1;i<=length($0);++i) { c=chrMap[substr($0, i, 1)]; if (c=="") {print "Character "substr($0, i, 1)" is not in the MAP." > "/dev/stderr"; exit 1;} printf(" %s",c); } print ""}
#END{print "# 1.0     <stroke>";print "<DUMMY> 1.0     <DUMMY>"}' > ${srcdir}/lexiconp.txt || exit 1

awk '{for(i=1;i<=NF;++i) print $i}' ${TEXT} | sort -u |
awk -v cm="${CM_FILE}" 'BEGIN{while (getline < cm)  { chrMap[$1]=$1 }}{printf("%-20s 1.0  ", $0); for(i=1;i<=length($0);++i) { c=chrMap[substr($0, i, 1)]; if (c=="") {print "Character "substr($0, i, 1)" is not in the MAP." > "/dev/stderr"; exit 1;} printf(" %s",c); } print ""}
END{print "<stroke> 1.0     <stroke>";print "<DUMMY> 1.0     <DUMMY>"}' > ${srcdir}/lexiconp.txt || exit 1
...
cat ${CM_FILE} > ${srcdir}/phones.txt
echo "<DUMMY>" >> ${srcdir}/phones.txt
#echo "<stroke>" >> ${srcdir}/phones.txt
