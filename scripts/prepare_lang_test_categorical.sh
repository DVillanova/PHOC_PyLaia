...
[ $# -ne 4 ] && {
    echo "Usage: ${0##*/} <fstLM> <langDir> <testDir> <Dummy-symb>" >&2;
    exit 1;
}
...
$lm | \
fstrmepsilon | \
fstarcsort --sort_type=ilabel > ${testd}/G_cat.fst

echo
set +e
fstisstochastic ${testd}/G_cat.fst || echo "[info]: G not stochastic." 1>&2
set -e
