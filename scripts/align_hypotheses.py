...
if len(sys.argv) >= 5:
    ...
    for l in gt_file.readlines():
        ...
        gt_dic[l_id] = [" " if c == "<space>" else c for c in transc.split()]
...
for h_idx in range(len(best_hyp_list)):
    best_hyp = [" " if c == "<space>" else c for c in best_hyp_list[h_idx][1].split()]
    correct_hyp = [" " if c == "<space>" else c for c in correct_hyp_list[h_idx][1].split()]
    counter += 1
    ...
    aligned_hyp = ["<space>" if c == " " else c for c in aligned_hyp]
    aligned_hyp_list.append(best_hyp_list[h_idx][0] + " " + " ".join(aligned_hyp))
